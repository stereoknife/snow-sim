...
namespace HPML
{
    public struct double2F : INativeDisposable
    {
        ...
        public NativeArray<double2> array;
        
        public double2F(double2F sf, Allocator allocator)
        {
            array = new NativeArray<double2>(sf.array.Length, allocator);
            dimension = sf.dimension;
            size = sf.size;
        }
        
        public double2F(doubleF sf, Allocator allocator, double2 initialValue = default)
        {
            array = new NativeArray<double2>(sf.field.Length, allocator);
        ...
        public double2F(int2 dimension, double3 size, Allocator allocator)
        {
            array = new NativeArray<double2>(vec.area(dimension), allocator);
        ...
        public double2 this[int key]
        {
            get => array[key];
            set => array[key] = value;
        }

        public double2 this[int2 key]
        {
            get => array[index(key)];
        ...
        public Texture2D ToTextureRGBA(Texture2D texture, bool makeNoLongerReadable = true)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                data[i] = new Color32((byte)c.x, (byte)c.y, 0, byte.MaxValue);
            }
        ...
        {
            [ReadOnly] public double2F field;
            ...
            public void Execute()
            {
                ...
                for (int i = 0; i < field.Length; i++)
                {
                    ...
                    texture[i] = new Color32((byte)c.x, (byte)c.y, 0, byte.MaxValue);
                }
            }
        }
    }
